...
def main():
    ...
    for agent in data.get("agents", []):
        agent_id = agent["id"]
        port = agent["port"]
        # module override: agents with hyphens in ID need a Python-safe module name
        module_name = agent.get("module", agent_id).replace("-", "_")
        conf = f"""[program:{agent_id}]
command=python -m agents.{module_name}.run
directory=/app
