...
from agents.nutrition.models import CoachingNote, FusedResult
...
class HealthCoachAgent:
    ...
    async def coach(self, fused: FusedResult, user_id: int | None = None) -> CoachingNote:
        ...
        calories_today = (daily_totals.get("calories_total") or 0) + fused.total_calories
        protein_today = (daily_totals.get("protein_total") or 0) + fused.total_protein
        ...
        try:
            ...
            if user_id is not None:
                row = await conn.fetchrow(
                    "SELECT calories_total, protein_total FROM daily_summaries "
            ...
            if user_id is not None:
                goal_row = await conn.fetchrow(
                    ...
                    "WHERE active = TRUE AND user_id = $1 LIMIT 1",
                    user_id,
                )
            else:
                goal_row = await conn.fetchrow(
