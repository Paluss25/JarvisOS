...
def create_drhouse_mcp_server(workspace_path: Path, redis_a2a=None):
    ...
    async def health_query(args: dict) -> dict:
        ...
        if _references_table(sql_lower, "activities") and _selects_column(sql_lower, "name"):
            corrections.append(
                "  'name' on table 'activities' → column does not exist "
                "(use 'type' for activity kind, or 'source'/'strava_activity_id' for identifiers)"
            )
        if _references_table(sql_lower, "activities") and (
            _selects_column(sql_lower, "start_time")
            or re.search(r"\border\s+by\s+start_time\b", sql_lower)
            or re.search(r"\bwhere\b.*(?<![.\w])start_time\b", sql_lower)
        ):
            corrections.append("  'start_time' on table 'activities' → use 'date'")
        ...
        for whoop_table in ("whoop_workouts", "whoop_sleeps", "whoop_cycles"):
            if _references_table(sql_lower, whoop_table) and (
                _selects_column(sql_lower, "start_time")
                or re.search(r"\border\s+by\s+start_time\b", sql_lower)
                or re.search(r"\bwhere\b.*(?<![.\w])start_time\b", sql_lower)
            ):
                corrections.append(f"  'start_time' on table '{whoop_table}' → use 'start_at'")
