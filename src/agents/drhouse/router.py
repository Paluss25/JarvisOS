...
@dataclass
class RoutePlan:
    consult: list[str] = field(default_factory=list)  # ["roger", "nutrition-director"]
...
def classify(message: str, has_image: bool = False, has_barcode: bool = False) -> RoutePlan:
    ...
    if has_image or has_barcode:
        plan.consult.append("nutrition-director")
    ...
    if has_nutrition and "nutrition-director" not in plan.consult:
        plan.consult.append("nutrition-director")
    if has_sport:
        plan.consult.append("roger")
