...
CHRO_BUILTIN_CRONS = [
    ...
    },
    {
        ...
        "prompt": (
            ...
            "Check payslips for a record in the current month using COALESCE(period_to, make_date(year, month, 1)) or month/year.\n"
    ...
    },
    {
        ...
        "prompt": (
            ...
            "Query: SELECT COALESCE(period_from, make_date(year, month, 1)) AS period_from, COALESCE(period_to, (make_date(year, month, 1) + INTERVAL '1 month - 1 day')::date) AS period_to, COALESCE(net_pay, net_amount) AS net_pay FROM payslips ORDER BY COALESCE(period_to, (make_date(year, month, 1) + INTERVAL '1 month - 1 day')::date) DESC LIMIT 2\n"
    ...
    },
    {
        ...
        "prompt": (
            ...
            "Query: SELECT ferie_remaining, snapshot_date FROM leave_snapshots ORDER BY snapshot_date DESC LIMIT 1\n"
    ...
    },
    {
        ...
        "prompt": (
            ...
            "   SELECT COUNT(*) FROM payslips WHERE COALESCE(period_to, (make_date(year, month, 1) + INTERVAL '1 month - 1 day')::date) < NOW() - INTERVAL '5 years'\n"
            "   If count > 0: 'GDPR: X payslips older than 5 years. Use delete_employee_data to remove.'\n\n"
            "2. Audit log entries older than 7 years:\n"
            "   SELECT COUNT(*) FROM hr_audit_log WHERE ts < NOW() - INTERVAL '7 years'\n"
            "   If count > 0: 'GDPR: X audit entries older than 7 years. Requires manual DBA action.'\n\n"
            "3. Expense items older than 10 years:\n"
            "   SELECT COUNT(*) FROM expense_items WHERE expense_date < NOW() - INTERVAL '10 years'\n"
